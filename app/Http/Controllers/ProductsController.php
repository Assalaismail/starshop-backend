...
class ProductsController extends ApiController
{
    ...
    public function generateProductSku($data): string
    {
        return strtoupper($data['season_code']) . strtoupper($data['subcategory_abbreviation']) . strtoupper($data['color']) . $data['size'];
    }
    ...
    public function addProducts(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'variations.*.quantity' => 'required|array',
        ...
        foreach ($validatedData['variations'] as $index => $variation) {
            ...
            foreach ($sizes as $sizeIndex => $size) {
                $quantity = $variation['quantity'][$sizeIndex];
                $isVariation = 1; // By default, set is_variation to 1 for variations

                $color = $variation['color'];

                // Fetch the color_abbreviation from the attributes table based on the color
                $colorAbbreviation = Attributes::where('title', $color)->value('abbreviation');

                if (!$colorAbbreviation) {
                    $colorAbbreviation = 'DEFAULT';
                }
                ...
                $sku = $this->generateProductSku($skuData);
